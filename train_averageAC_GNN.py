...
def main():
    ...
    GNNmodel = GNN(nfeat=features.shape[1], args=args)
    ACmodel = AverageAC()
    ...
    features_embedding = torch.zeros(features.shape).cuda()
    with torch.no_grad():
        ...
        for i, sub_node in enumerate(sub_nodes):
            ...
            feature_src_AC = ACmodel(subgraph_adj_list[i].cuda(), features[sub_node][feat_keep_idx_sub])
            features_embedding[sub_node[feat_drop_idx_sub]] = feature_src_AC[feat_drop_idx_sub]
            features_embedding[sub_node[feat_keep_idx_sub]] = features[sub_node][feat_keep_idx_sub]
    ...
    for epoch in range(args.epochs):
        ...
        print("Epoch: {:04d}, Cy_loss: {:.4f}".format(epoch, Cy_loss.item()))
        ...
        if acc_test > args.acc and roc_test > args.roc:
            ...
            if best_fair > parity + equality:
                ...
                torch.save(GNNmodel,"average_GNN_epoch{:04d}_acc{:.4f}_roc{:.4f}_par{:.4f}_eq_{:.4f}".format(epoch,acc_test.item(),roc_test
                                                                                     ,parity,equality))

            print("=================================")
            log = "Epoch: {:04d}, cls: {:.4f}".format(epoch,  cls_loss.item())
            with open('average_log.txt', 'a') as f:
                f.write(log)
