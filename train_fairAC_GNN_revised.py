...
from models.FairAC import FairAC, FairAC2, GNN
...
def parser_args():
    ...
    parser.add_argument('--lambda0', type=float, default=0.001,
                        help='The hyperparameter of lambda for loss tripet')
...
def main():
    ...
    for epoch in range(args.epochs+1000):
        t = time.time()
        GNNmodel.train()
        ACmodel.train()
...
# todo: 1. change csen_adv_loss to two parts, maybe because Csen is not good enough.
#        2. try new ac , maybe because of too much topological unfair or label unfair
        if epoch < args.epochs and not args.load:
            ...
            # tripet loss
            min_values = features_train[feat_keep_idx].min(axis=0)[0]
            max_values = features_train[feat_keep_idx].max(axis=0)[0]
            features_mean = (max_values+min_values)/2
            features_mean = torch.ones([transformed_feature.shape[0],features_mean.shape[0]], device=features_mean.device) * features_mean
            loss_tripet = args.lambda0 *ACmodel.loss(features_mean, transformed_feature)
            # base AC finished###############

            # pretrain AC model
            if epoch < 200:
                # ###############pretrain AC model ##########################
                print("Epoch: {:04d}, loss_ac: {:.4f},loss_reconstruction: {:.4f}, loss_tripet: {:.4f}"
                      .format(epoch, loss_ac.item(), loss_reconstruction.item(), loss_tripet.item()))
                AC_loss = loss_reconstruction + loss_ac - loss_tripet
                AC_loss.backward()
            ...
            # sens_confusion = torch.ones(sens_train.shape, device=sens_train.device, dtype=torch.float32)/2
            # Csen_adv_loss = criterion(sens_prediction, sens_confusion.unsqueeze(1))
            ...
            # sensitive optimizer.step
            # AC optimizer.step
            AC_loss = args.lambda2*(Csen_adv_loss -Csen_loss)+loss_reconstruction + args.lambda1*loss_ac - loss_tripet
            # 1*(Csen_adv_loss -Csen_loss)+
            AC_loss.backward()
            ACmodel.optimizer_AC.step()

            if epoch < args.epochs:
                print("Epoch: {:04d}, loss_ac: {:.4f}, loss_reconstruction: {:.4f}, Csen_loss: {:.4}, Csen_adv_loss: {:.4f}, loss_tripet: {:.4f}"
                        .format(epoch, loss_ac.item(), loss_reconstruction.item(), Csen_loss.item(), Csen_adv_loss.item(),loss_tripet.item()
                                ))
            ...
            if epoch > 1000 and epoch % 200 == 0:
                ...
                for sub_epoch in range(1000):
                    ...
                    # feat_emb, y = GNNmodel(G, features_embedding)
                    #
                    # Cy_loss = GNNmodel.criterion(y[idx_train], labels[idx_train].unsqueeze(1).float())
                    # Cy_loss.backward()
                    #
